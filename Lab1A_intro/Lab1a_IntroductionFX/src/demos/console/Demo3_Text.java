...
public class Demo3_Text extends BaseConsole{
    ...
    private void sortFR(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText(String.join(nL, words) + nL);
        String[] ws = ta1.getText().split(nL);
        Collator collatorFR= Collator.getInstance(new Locale("fr_BE"));
        Arrays.sort(ws, collatorFR);  // Rikiavimas prancūziškai
        ta2.appendText("***** rikiavimas pagal prancūziškas taisykles\n");
        ta2.appendText(String.join(nL, ws) + nL); 
    }
...
// http://www.java2s.com/Tutorials/Java/Java_Regular_Expression/0070__Java_Regex_Groups.htm    
    ...
    ***************************************************/
    void regexExamples(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            //ta1.appendText("\\p{IsLatin}+<>pirmas=1;antras>-22:trečias:333viskas" + nL); // a
           // ta1.appendText("[a-ŽA-Ž]*[ĄąČčĘęĖėĮįŠšŲųŪūŽž][a-žA-Ž]*<>pirmas=1;antras>-22:trečias:viskas" + nL); // b 
             ta1.appendText("(?<=\\().+?(?=\\))<>pirmas=1;antra(s>-22:trečias):333viskas" + nL); // c
        
            
            //ta1.appendText("[-]<>pirmas=1;antras>-22:trečias:333viskas" + nL);
    ...
    public void createControls() {
        ...
        addButton("sortLT",  action -> sortLT());  
         addButton("sortFR",  action -> sortFR());  
